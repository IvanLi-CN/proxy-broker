# Forward Auth 身份识别、管理员授权与 Project API Key

## Goal

在 `proxy-broker` 内部落地完整的鉴权与授权链路：Forward Auth 只负责识别人类用户身份，应用自身负责管理员判定、开发模式身份注入，以及 Project 级机器 API Key 的发行、校验和访问边界。
...
- 新增 Project 级 API Key 的存储、一次性签发、撤销、`last_used_at` 更新与后端校验。
- 把 `/`、静态资源、project 管理接口、project 业务接口收敛到明确的授权矩阵。
- 在 Web 管理台显示当前身份，并提供管理员可用的 Project API Key 管理界面。
...
- 不实现内建用户名密码登录。
- 不实现外部 JWT/JWKS 机器鉴权。
- 不实现多 project 共享 API Key。
- 不引入人类用户数据库或更细粒度的人类 project 权限模型。
...
- Project 级 API Key 仅允许访问绑定 project 的业务接口；跨 project 访问返回 `403 project_access_denied`。
- API Key 不能访问 `/`、静态资源、`/api/v1/projects` 与 `/api/v1/projects/{project_id}/api-keys*`。
- API Key 创建响应只返回一次明文 secret，后续列表仅返回 `key_id/name/prefix/project_id/created_by/created_at/last_used_at/revoked_at`。
- SQLite 与 Memory store 都支持 API Key 的增查撤销与 `last_used_at` 更新。
- Web 管理台展示当前 principal，并允许管理员创建与撤销当前 project 的机器密钥。
...
- 后端新增 `AuthConfig`、请求级 principal 解析中间件、开发模式注入逻辑，以及管理员/Project 访问守卫。
- 人类身份头只会在请求来源命中 `PROXY_BROKER_AUTH_TRUSTED_PROXIES` 时生效，避免直连调用伪造管理员身份。
- 路由矩阵固定为：`/healthz` 公开；`/` 与嵌入静态资源仅管理员人类或开发 principal 可访问；`/api/v1/projects` 与 `/api/v1/projects/{project_id}/api-keys*` 仅管理员人类或开发 principal 可访问；`/api/v1/projects/{project_id}/subscriptions|refresh|ips|sessions` 允许管理员人类、开发 principal 或匹配 project 的 API Key 访问。
- API Key secret 采用 `pbk_<key_id>_<random>` 形状，落库仅保存 `salt + sha256(secret)`，校验使用常量时间比较。
- Web 管理台新增身份展示与 Project API Key 管理卡片，支持一次性显示最新签发的 secret。
