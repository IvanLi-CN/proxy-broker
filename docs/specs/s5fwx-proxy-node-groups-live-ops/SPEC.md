...
- 现有元信息刷新、测速与会话打开都强绑定 project runtime，无法在同一代理工作区里统一运营全局与 project 节点。
...
- 统一运行时为单一共享 runtime，使 Global/Project 节点运营与测速都基于同一个 controller。
- 为节点级刷新元信息、五次广度优先测速、Project 级节点定向建会话提供稳定 API 与实时推送。
...
- `/proxies` 的 grouped node UI、批量操作、Project-only 会话创建与实时推送
...
- Project `/proxies` 只展示当前 project effective pool 中可用的节点，并支持节点级与批量创建会话。
...
- runtime 内部代理别名必须与 `node_id` / `node_id+ip` 绑定，避免跨 import / 跨 project 同名节点冲突。

### SHOULD

- 节点元信息真相源提升到 inventory-node/IP 层，并尽量复用历史 project 级 `ip_records` / `probe_records` 做初次回填。
...
- Project 视图下若节点当前不属于 effective pool，不允许创建会话。
...
- Project 默认使用 `resolved_ips[0]` 的约束已冻结
...
- `play` / interaction coverage to add/update: Global batch probe、自动测速间隔设置、成功/失败/空样本状态、tooltip 最新 10 条历史、Project batch create-session、compact 会话候选行、empty / access-denied / loading states
...
- `evidence_note=Shows the node switch candidate dialog with latest-sample latency status, tooltip history, and the current session/project last-used label kept on one row with the timestamp truncating under constrained width.`
...
- `sensitive_exclusion=N/A`
- `submission_gate=approved`
- `story_id_or_title=Pages/ProxiesPage/ProjectCatalog`
- `state=project grouped node batch selection`
- `evidence_note=Shows the project grouped node table after selecting nodes across two import groups: the global import group is indeterminate, the selected-node toolbar exposes refresh/probe/create-session actions, and the Chinese table layout does not overlap.`

PR: include
![Project grouped node batch selection](./assets/proxies-batch-selection.png)
...
- `submission_gate=approved`
- `story_id_or_title=Pages/ProxiesPage/ProjectCatalog`
- `state=project grouped node catalog`
- `evidence_note=Shows the project-scoped grouped node list with inherited global nodes, project-local imports, create-session actions, and the local-import delete affordance visible together in one baseline review surface.`

PR: include
![Project grouped node catalog](./assets/proxies-project-catalog.png)
...
- `sensitive_exclusion=N/A`
- `submission_gate=approved`
- `story_id_or_title=Pages/ProxiesPage/ProjectCreateSessionDialog`
...
PR: include
![Project single node create-session dialog](./assets/proxies-project-create-session-dialog.png)
...
- `sensitive_exclusion=N/A`
- `submission_gate=approved`
- `story_id_or_title=Pages/ProxiesPage/ProjectBatchCreateDialog`
...
PR: include
![Project batch create-session dialog](./assets/proxies-project-batch-create-dialog.png)
...
- `sensitive_exclusion=N/A`
- `submission_gate=approved`
- `story_id_or_title=Pages/ProxiesPage/ProjectDeleteConfirmDialog`
- `state=project import delete confirmation`
- `evidence_note=Shows the destructive confirmation step before removing a project-local import, including the import label, kind, and scope summary needed for the operator to verify the target.`

PR: include
![Project delete confirmation dialog](./assets/proxies-project-delete-confirm-dialog.png)
...
- 以单一共享 runtime 承载所有 inventory 节点与跨 project listeners，彻底去掉按 project 分 controller 的限制。
- 保留 project effective pool 作为业务归属与筛选语义，但 runtime 内部代理命名统一切换到 node-based alias。
- 节点目录与实时运营流都建立在 inventory-node/IP 真相源之上，旧 project 级元信息仅用于初次回填与兼容旧功能。
...
- 假设（需主人确认）：Project 级节点定向建会话固定使用主 IP `resolved_ips[0]`，不做交互式选择。
