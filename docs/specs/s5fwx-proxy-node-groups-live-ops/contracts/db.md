...
- Recent-sample reads must be bounded in the store layer:
  - pair reads filter by `(node_id, ip)` and return at most the requested limit
  - global reads apply per-pair limiting before rows leave SQLite
